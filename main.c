...
//end of pre-define values

//students data type define
typedef struct {
char stuname[100];
char id[9];
float gpa;
...
//end students data type define

//faculty data type define
typedef struct {
char stuname[100];
char id[9];
char dept[10];
...
void addStud() {
    ...
    printf("Name of Stud: ");
    getchar();
    fgets(s.stuname, 100, stdin);
    s.stuname[strcspn(s.stuname, "\n")] = '\0';
...
void addFaculty() {
    ...
    getchar();
    fgets(f.stuname, 100, stdin);
    f.stuname[strcspn(f.stuname, "\n")] = '\0';
    ...
    for (int attempts = 0; attempts < 3; attempts++) {
        printf("dept: ");
        scanf("%s", f.dept);
        if (validatedept(f.dept)) break;
        printf("Invalid dept. Try again.\n");
        if (attempts == 2) {
            printf("You have tried too many times. Goodbye.\n");
            return;
        }
...
void searchFaculty() {
    ...
    for (int i = 0; i < facultyCount; i++) {
        if (strcmp(faculty[i].id, id) == 0) {
            ...
            printf("---------------------------------------------------------------------------\n");
            printf("%s\n%s dept, %s\n", faculty[i].stuname, faculty[i].dept, faculty[i].rank);
            printf("---------------------------------------------------------------------------\n");
            return;
        }
    }
